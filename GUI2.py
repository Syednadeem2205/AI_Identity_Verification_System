...
def load_yolov8_model():
    model = YOLO('yolov5su.pt')  # Ensure you have the correct path to the model
    return model
        ...
        for result in results[0].boxes:
            ...
            try:
                ...
                if face_encodings:
                    ...
                    cv2.putText(frame, f"Confidence: {confidence * 100:.2f}%", (right, top + 60), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
...
def main_window():
    ...
    tk.Button(frame, text="Exit", command=exit_application, **button_style).grid(row=4, column=0, columnspan=2, pady=10)
...
def login_window():
    ...
    title_label = tk.Label(login, text="AI Identity Verification System", font=('Impact', 40, 'bold'), fg="white", bg="#1e1e1e")
    title_label.pack(pady=50)

    description_label = tk.Label(login, text="Secure Identity Verification Using Face Detection", font=('Arial', 18), fg="white", bg="#1e1e1e")
    ...
    tk.Button(frame, text="Exit", command=exit_application, **button_style).grid(row=3, column=0, columnspan=2, pady=10)
